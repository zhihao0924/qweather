...
func New(cfg Config) (*Client, error) {
	...
	if strings.TrimSpace(cfg.APIKey) == "" && cfg.TokenProvider == nil {
...
func (c *Client) DoJSON(ctx context.Context, method, path string, query Params, out any) error {
...
func (c *Client) newRequest(ctx context.Context, method, path string, query Params) (*http.Request, error) {
	u := *c.baseURL
	u.Path = strings.TrimRight(c.baseURL.Path, "/") + "/" + strings.TrimLeft(path, "/")

	values := u.Query()
	for key, entries := range query.values {
